...
| steps | `.steps`（推荐 `.chapter-practice` 内 `ol.steps.steps-operate`） | 动手练习·操作步骤 |
...
对已存在的 `chapters/<id>.html` 检查缺失块（Gate 1 内 `validate-tutorial.mjs`；命令见 [delivery-review.md](delivery-review.md)）。
